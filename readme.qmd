---
title: "Highways width based on LIDAR data"
...
#| message: false
#| warning: false
#| fig-cap: Orthophoto view of the road with vertices added (shown in red).
...
```
The approach is to find a point in the middle between pair of vertices and create a 20-m long and 6 m wide "transect" at this point: 10 m left and 10 m right from the road. @fig-transect_overviev shows an example.

```{r}
#| label: fig-transect_overviev
#| code-fold: true
#| fig-cap: "Analized area of the road: [red point]{style=\"color:red;\"} -- middle of the linestring, green line -- perpendicular line with length of 20 m, gray area -- analized area: 20x6 m."
...
```{r}
#| label: fig-transect_lidar
#| message: false
#| warning: false
#| code-fold: true
#| fig-cap: "LIDAR data plots in analyzed area."
#| fig-subcap: 
#|    - "points height"
#|    - "points classification"
#|    - "points intensity"
#|    - "X-Y view of LIDAR points by classification"
##| layout-ncol: 2
##| column: body-outset
...
```{r}
#| label: fig-transect_centered
#| message: false
#| warning: false
#| code-fold: true
#| fig-cap: "LIDAR data plots centered on midpoint"
#| fig-subcap: 
#|    - "points height with intensity"
#|    - "X-Y view of points intensity"
##| layout-ncol: 2
##| column: body-outset
...
```
```{r}
#| label: tbl-calculated_data
#| output: false
#| code-fold: true
#| tbl-cap: "Subset of mean intensity (Im) and height (Zm) of (un)classified strips across transects. s -- distance from mid point."
...
The calculated width is `r h_width` meters. @fig-road_calc shows the LIDAR points with calculated road boundaries. You can notice the shift of the boundaries in the relation to 0 which means that OpenStreetMap line is not drawn centrally to the LIDAR data. The displacement is by `r abs(h_max) - (abs(h_min) + abs(h_max))/2` meters.
...
#| warning: false
#| code-fold: true
#| fig-cap: "LIDAR data plots with road boundaries marked"
